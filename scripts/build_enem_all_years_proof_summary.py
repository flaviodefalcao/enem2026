...
def resolve_student_id_column(parquet_path: Path) -> str:
    sample = pd.read_parquet(parquet_path, columns=None).head(0)
    if "NU_SEQUENCIAL" in sample.columns:
        return "NU_SEQUENCIAL"
    if "ID_ALUNO" in sample.columns:
        return "ID_ALUNO"
    raise KeyError(f"Parquet sem coluna de identificação reconhecida: {parquet_path}")


def load_area_frame(input_dir: Path, area_code: str) -> pd.DataFrame:
    parquet_path = input_dir / area_code / "06_gold_a_student_wide.parquet"
    student_id = resolve_student_id_column(parquet_path)
    return pd.read_parquet(
        parquet_path,
        columns=[student_id, *SCORE_COLUMNS],
    )
...
def bucketize_distribution(series: pd.Series, *, start: int, stop: int, step: int):
...
def bucketize_accuracy_vs_score(
    ...
    *,
    ...
    bins = list(range(start, stop + step, step))
    labels = build_bucket_labels(start, stop, step)
    data["bucket"] = pd.cut(data[score_col], bins=bins, labels=labels, right=False, include_lowest=True)
    grouped = (
...
def build_payload(year: int):
    input_dir = ROOT / f"output_enem_all_areas_{year}"
    area_frames = {area_code: load_area_frame(input_dir, area_code) for area_code in AREA_META}
    id_column = "NU_SEQUENCIAL" if "NU_SEQUENCIAL" in area_frames["LC"].columns else "ID_ALUNO"
    ...
    for area_code, frame in area_frames.items():
        overall_base = overall_base.merge(
            frame[[id_column, "total_acertos"]].rename(
    ...
    for area_code, meta in AREA_META.items():
        ...
        areas_payload[meta["slug"]] = {
            ...
            "averageAccuracy": round(float((frame["total_acertos"].mean() / meta["questionCount"]) * 100), 1),
            "scoreDistribution": bucketize_distribution(frame["NU_NOTA_AREA"], start=300, stop=1000, step=50),
    ...
    return {
        ...
        "generalScoreDistribution": bucketize_distribution(overall_base["NU_NOTA_MEDIA_5"], start=200, stop=1000, step=50),
        "essayDistribution": bucketize_distribution(overall_base["NU_NOTA_REDACAO"], start=0, stop=1000, step=100),
        "topStudents": {
            "threshold": 800,
...
def main() -> None:
    ...
    for year in DEFAULT_YEARS:
        ...
        output_path = OUTPUT_ROOT / f"enem-{year}-proof-summary.json"
